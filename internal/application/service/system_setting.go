...
// suffix DIXIAN_KNOWLEDGE_REDIS_NAMESPACE so two deployments sharing one Redis
...
func pubsubChannel() string {
	if ns := strings.TrimSpace(os.Getenv("DIXIAN_KNOWLEDGE_REDIS_NAMESPACE")); ns != "" {
...
var registry = map[string]settingSpec{
	...
	"auth.default_tenant_mode": {
		Type:     "string",
		EnvName:  "DIXIAN_KNOWLEDGE_AUTH_DEFAULT_TENANT_MODE",
	...
	// EnvName 与启动时读取的 DIXIAN_KNOWLEDGE_TENANT_MAX_OWNED_PER_USER 相同，
	...
	"tenant.max_owned_per_user": {
		Type:     "int",
		EnvName:  "DIXIAN_KNOWLEDGE_TENANT_MAX_OWNED_PER_USER",
		Default:  int64(10),
		Category: "tenant",
	...
	"tenant.self_service_creation_enabled": {
		Type:     "bool",
		EnvName:  "DIXIAN_KNOWLEDGE_TENANT_SELF_SERVICE_CREATION_ENABLED",
		Default:  true,
		Category: "tenant",
	...
	"tenant.default_storage_quota_gb": {
		Type:     "int",
		EnvName:  "DIXIAN_KNOWLEDGE_TENANT_DEFAULT_STORAGE_QUOTA_GB",
		Default:  int64(10),
		Category: "tenant",
	...
	// 也可通过 DIXIAN_KNOWLEDGE_TENANT_AUTO_CREATE_API_KEY 设置。默认 false 保持
	// current, safer no-implicit-key behaviour. Read at create time only.
	"tenant.auto_create_api_key": {
		Type:     "bool",
		EnvName:  "DIXIAN_KNOWLEDGE_TENANT_AUTO_CREATE_API_KEY",
		Default:  false,
		Category: "tenant",
	...
	"asynq.core_concurrency": {
		Type:            "int",
		EnvName:         "DIXIAN_KNOWLEDGE_ASYNQ_CORE_CONCURRENCY",
	...
	"asynq.postprocess_concurrency": {
		Type:            "int",
		EnvName:         "DIXIAN_KNOWLEDGE_ASYNQ_POSTPROCESS_CONCURRENCY",
	...
	"asynq.enrichment_concurrency": {
		Type:            "int",
		EnvName:         "DIXIAN_KNOWLEDGE_ASYNQ_ENRICHMENT_CONCURRENCY",
	...
	"asynq.maintenance_concurrency": {
		Type:            "int",
		EnvName:         "DIXIAN_KNOWLEDGE_ASYNQ_MAINTENANCE_CONCURRENCY",
	...
	"asynq.shared_concurrency": {
		Type:            "int",
		EnvName:         "DIXIAN_KNOWLEDGE_ASYNQ_SHARED_CONCURRENCY",
	...
	// 对应 DIXIAN_KNOWLEDGE_WIKI_ASYNQ_CONCURRENCY，默认值为 8。
	"asynq.wiki_concurrency": {
		Type:            "int",
		EnvName:         "DIXIAN_KNOWLEDGE_WIKI_ASYNQ_CONCURRENCY",
	...
	// 对应 DIXIAN_KNOWLEDGE_MODEL_MAX_CONCURRENCY，默认值为 32；0 或负数关闭
	// the default cap.
	"model.max_concurrency": {
		Type:     "int",
		EnvName:  "DIXIAN_KNOWLEDGE_MODEL_MAX_CONCURRENCY",
		Default:  int64(32),
		Category: "worker",
...
func (s *systemSettingService) applyModelMaxConcurrency(ctx context.Context) {
	limit := int(s.GetInt(ctx, "model.max_concurrency", "DIXIAN_KNOWLEDGE_MODEL_MAX_CONCURRENCY", 32))
